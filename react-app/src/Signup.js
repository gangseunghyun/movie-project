...
function Signup({ onSignupSuccess, onSwitchToLogin }) {
  ...
  const sendVerificationCode = async () => {
    ...
    try {
      await axios.post('http://localhost:8080/api/mail/send-verification', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/users/join', formData);
  ...
  const getRecommendNicknames = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/users/recommend-nickname');
